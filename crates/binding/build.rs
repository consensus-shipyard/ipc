...
fn main() -> color_eyre::Result<()> {
    ...
    if let Ok(val) = std::env::var(SKIP_ENV_VAR_NAME) {
    	...
    	if val == "true" || val == "1" || val == "" {
			println!("cargo:warn=Skipping binding generation since {} is set by the user", SKIP_ENV_VAR_NAME);
			return Ok(());
        }
    }
    ...
    let workspace_dir = crate_dir.clone()
    ...
    writeln!(lib,"// The list of contracts that need to convert common types between each other")?;
