...
RUN find /app/crates -type f \! -name "Cargo.*" | xargs rm -rf

# Construct dummy sources. Add a print to help debug the case if we failed to properly replace the file.
RUN echo "fn main() { println!(\"I'm the dummy.\"); }" > /app/crates/node/app/src/main.rs && \
  for crate in $(find /app/crates -name "Cargo.toml" | xargs dirname | grep -v infra | grep -v node_modules | grep /); do \
...
RUN \
  ...
  --mount=type=cache,target=/app/crates/target,sharing=locked \
  set -eux; \
  case "${TARGETARCH}" in \
  amd64) ARCH='x86_64'  ;; \
  arm64) ARCH='aarch64'; \
...
RUN find /app/crates -type f \( -wholename "**/src/lib.rs" -o -wholename "**/src/main.rs" \) | xargs touch
...
RUN \
  ...
  cargo install --locked --root output --path node/app --target ${ARCH}-unknown-linux-gnu ; \
