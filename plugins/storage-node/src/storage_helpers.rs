...
use fendermint_actor_storage_blobs_shared::blobs::{
...
use fendermint_actor_storage_blobs_shared::method::Method::{
    GetAddedBlobs, GetBlobStatus, GetPendingBlobs, GetStats,
};
use fendermint_actor_storage_blobs_shared::{GetStatsReturn, BLOBS_ACTOR_ADDR};
...
pub fn get_added_blobs<DB>(
    state: &mut FvmExecState<ReadOnlyBlockstore<DB>, fendermint_module::NoOpModuleBundle>,
    size: u32,
) -> Result<Vec<BlobItem>>
where
...
pub fn get_blobs_stats<DB>(state: &mut FvmExecState<DB, fendermint_module::NoOpModuleBundle>) -> Result<GetStatsReturn>
where
    DB: Blockstore + Clone + 'static + Send + Sync,
{
    let msg = create_implicit_message(
...
pub fn set_read_request_pending<DB, M>(state: &mut FvmExecState<DB, M>, id: Hash) -> Result<FvmApplyRet>
where
...
pub fn read_request_callback<DB, M>(
...
pub fn close_read_request<DB, M>(state: &mut FvmExecState<DB, M>, id: Hash) -> Result<FvmApplyRet>
where
    DB: Blockstore + Clone + 'static + Send + Sync,
    M: fendermint_module::ModuleBundle,
{
...
where
    F: FnOnce(&mut FvmExecState<ReadOnlyBlockstore<DB>, fendermint_module::NoOpModuleBundle>) -> Result<R>,
    DB: Blockstore + Clone + 'static + Send + Sync,
{
    state.state_tree_mut_with_deref().begin_transaction();
    let result = f(state);
    state
