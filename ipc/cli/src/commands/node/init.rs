...
use ipc_runner::process_manager::ProcessManager;
use ipc_runner::services::comet_bft;
use std::io::Write;
...
async fn init_comet_bft(home: &Path) -> anyhow::Result<()> {
    ...
    let process_manager = ProcessManager::new("cometbft".into(), vec!["init", "--home", &home]);
    process_manager.execute().await
}
