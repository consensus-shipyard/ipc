...
library LibMinPQ {
    ...
    function insert(MinPQ storage self, ProofOfPower storage proofOfPower, address validator) internal {
        ...
        uint256 power = proofOfPower.getConfirmedPower(validator);
        swim({self: self, proofOfPower: proofOfPower, pos: size, value: power});
    }

    /// @notice Pop the minimal value in the priority queue.
    function pop(MinPQ storage self, ProofOfPower storage proofOfPower) internal {
        ...
        uint256 power = self.inner.getPower(proofOfPower, 1);
        sink({self: self, proofOfPower: proofOfPower, pos: 1, value: power});
    }

    /// @notice Reheapify the heap when the validator is deleted.
    function deleteReheapify(MinPQ storage self, ProofOfPower storage proofOfPower, address validator) internal {
        ...
        uint256 val = self.inner.getPower(proofOfPower, pos);
        swim({self: self, proofOfPower: proofOfPower, pos: pos, value: val});

        // sink pos down in case updated pos is larger
        val = self.inner.getPower(proofOfPower, pos);
        sink({self: self, proofOfPower: proofOfPower, pos: pos, value: val});
    }

    /// @notice Reheapify the heap when the collateral of a key has increased.
    function increaseReheapify(MinPQ storage self, ProofOfPower storage proofOfPower, address validator) internal {
        uint16 pos = self.inner.getPosOrRevert(validator);
        uint256 val = proofOfPower.getConfirmedPower(validator);
        sink({self: self, proofOfPower: proofOfPower, pos: pos, value: val});
    }

    /// @notice Reheapify the heap when the collateral of a key has decreased.
    function decreaseReheapify(MinPQ storage self, ProofOfPower storage proofOfPower, address validator) internal {
        uint16 pos = self.inner.getPosOrRevert(validator);
        uint256 val = proofOfPower.getConfirmedPower(validator);
        swim({self: self, proofOfPower: proofOfPower, pos: pos, value: val});
    }
    ...
    function min(MinPQ storage self, ProofOfPower storage proofOfPower) internal view returns (address, uint256) {
        self.inner.requireNotEmpty();

        address addr = self.inner.posToAddress[1];
        uint256 power = proofOfPower.getConfirmedPower(addr);
        return (addr, power);
    }
     ...
     ****************************************************************************/
    function swim(MinPQ storage self, ProofOfPower storage proofOfPower, uint16 pos, uint256 value) internal {
        uint16 parentPos;
        uint256 parentPower;
        ...
        while (pos > 1) {
            // parentPos = pos / 2;
            parentPos = pos >> 1;
            parentPower = self.inner.getPower(proofOfPower, parentPos);
    ...
    function sink(MinPQ storage self, ProofOfPower storage proofOfPower, uint16 pos, uint256 value) internal {
        ...
        while (childPos <= size) {
            if (childPos < size) {
                ...
                (childPos, childPower) = smallerPosition({
                    self: self,
                    proofOfPower: proofOfPower,
                    pos1: childPos,
                    pos2: childPos + 1
                });
            } else {
                childPower = self.inner.getPower(proofOfPower, childPos);
            }
    ...
    function smallerPosition(
        MinPQ storage self,
        ProofOfPower storage proofOfPower,
        uint16 pos1,
        uint16 pos2
    ) internal view returns (uint16, uint256) {
        uint256 value1 = self.inner.getPower(proofOfPower, pos1);
        uint256 value2 = self.inner.getPower(proofOfPower, pos2);
