...
contract GatewayActorDiamondTest is Test, IntegrationTestBase {
    ...
    function testGatewayDiamond_createBottomUpCheckpoint() public {
        ...
        // now one can create a checkpoint that is not a multiple of checkpoint period
        checkpoint = BottomUpCheckpoint({
            subnetID: gatewayDiamond.getter().getNetworkName(),
            blockHeight: d + d / 2,
        ...
        BottomUpCheckpoint memory c = gatewayDiamond.getter().bottomUpCheckpoint(d + d / 2);
        require(c.blockHeight == d + d / 2, "out height incorrect");
    }
