            ...
            match listener.accept().await {
                Ok((socket, _addr)) => {
                    ...
                    tokio::spawn(async move {
                        if let Err(e) = conn.run(read, write).await {
                            tracing::error!(error = %e, "ABCI connection terminated");
                        }
                    });
                }
                Err(e) => {
        ...
        loop {
            select! {
                req = request_stream.next() => {
                    ...
                    let request = match Request::try_from(proto) {
                        Ok(request) => request,
                        Err(e) if e.to_string() == "missing data field" => {
                            // Some CometBFT peers can send an empty Request frame for flush.
                            // Treat it as Flush to keep the connection alive.
                            tracing::warn!("received empty ABCI request; treating as Flush");
                            Request::Flush
                        }
