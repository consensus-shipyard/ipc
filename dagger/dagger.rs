...
use dagger_sdk::{logging::{StdLogger, TracingLogger}, Container, ContainerBuildOpts, ContainerBuildOptsBuilder, ContainerWithMountedCacheOpts, ContainerWithMountedCacheOptsBuilder, ContainerWithMountedDirectoryOpts, DaggerConn, Directory, File, HostDirectoryOpts, Service};
use rand::Rng;
...
async fn prepare_anvil_service(client: DaggerConn) -> eyre::Result<Service> {
    ...
    let container = client
        .container()
        .build_opts(context_dir, ContainerBuildOptsBuilder::default().dockerfile("docker/Dockerfile").build()?);
...
fn repo_root_dir(client: &DaggerConn) -> Directory {
...
fn define_crates_container(client: DaggerConn) -> Result<Container> {
    ...
    Ok(client
        ...
        .with_exec(cmd("apt-get install -y build-essential clang cmake protobuf-compiler"))
...
async fn prepare_fendermint_two_stage_build(client: DaggerConn) -> eyre::Result<Container> {
    let repo_root_dir = repo_root_dir(&client);
    
    let fendermint_dir = client
        .host()
        .directory("fendermint");
    ...
    let crates_def = define_crates_container(client.clone())?
        .with_file("/foo", output); // TODO more files    
...
async fn main() -> eyre::Result<()> {
    ...
    dagger_sdk::connect_opts(
     ...
     |client| async move {
        let _fendermint_featherweight = prepare_fendermint_two_stage_build(client).await.unwrap();
