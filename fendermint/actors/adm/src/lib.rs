...
use cid::Cid;
use fil_actor_adm::{AdmActor, Method};
use fil_actors_runtime::{
    runtime::{builtins::Type, ActorCode, Runtime},
    ActorDowncast, ActorError, ADM_ACTOR_ID,
};
...
pub const IPC_ADM_ACTOR_NAME: &str = "adm";
pub const IPC_ADM_ACTOR_ID: ActorID = ADM_ACTOR_ID;
...
//
// impl MachineKind {
//     fn actor_type(&self) -> Type {
//         match self {
//             MachineKind::ObjectStore => Type::ObjectStore,
//             MachineKind::Accumulator => Type::Accumulator,
//         }
//     }
// }
...
pub enum ExtraMethods {
    CreateObjectStore = frc42_dispatch::method_hash!("CreateObjectStore"),
...
impl Actor {
    ...
    fn get_machine_code(rt: &impl Runtime, kind: u64) -> Result<Cid, ActorError> {
        let st: State = rt.state()?;
        match st.get_machine_code(rt.store(), kind)? {
            Some(code) => Ok(code),
            None => Err(ActorError::forbidden(String::from(
    ...
    fn update_deployers(rt: &impl Runtime, deployers: Vec<Address>) -> Result<(), ActorError> {
        ...
        rt.transaction(|st: &mut State, rt| {
            st.set_deployers(rt.store(), deployers).map_err(|e| {
                e.downcast_default(ExitCode::USR_ILLEGAL_STATE, "failed to set deployers")
            })
        })?;
...
impl ActorCode for Actor {
        ...
        RT::Blockstore: Blockstore + Clone,
    {
        if method == Method::Constructor as u64 {
            ...
            let code = Self::get_machine_code(rt, 0)?;

            AdmActor::invoke_method(rt, method, params)
        }
    }
}
