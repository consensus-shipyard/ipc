...
impl<DB: Blockstore + Clone> GatewayCaller<DB> {
    ...
    /// Checks if the validator set has changed since the last created bottom up checkpoint
    pub fn has_validator_set_changed(&self, state: &mut FvmExecState<DB>) -> anyhow::Result<bool> {
    ...
    pub fn create_bottom_up_checkpoint(
        ...
        tracing::debug!("power_table: {power_table:?}");
    ...
    pub fn current_power_table(
        ...
        tracing::info!(?membership, "current membership");
        tracing::info!(scale = ?state.power_scale(), "power scale");
