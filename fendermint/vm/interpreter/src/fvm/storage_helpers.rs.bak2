...
pub fn get_added_blobs<DB>(
    state: &mut FvmExecState<ReadOnlyBlockstore<DB>, DefaultModule>,
    size: u32,
) -> Result<Vec<BlobItem>>
where
...
pub fn get_blobs_stats<DB>(state: &mut FvmExecState<DB, DefaultModule>) -> Result<GetStatsReturn>
where
    DB: Blockstore + Clone + 'static + Send + Sync,
{
    let msg = create_implicit_message(
...
where
    F: FnOnce(&mut FvmExecState<ReadOnlyBlockstore<DB>, DefaultModule>) -> Result<R>,
    DB: Blockstore + Clone + 'static + Send + Sync,
{
