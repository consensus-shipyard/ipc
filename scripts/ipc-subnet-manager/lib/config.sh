...
# Convert subnet ID to the filename-safe format used by ipc-cli generated configs.
sanitize_subnet_id_for_filename() {
    local subnet_id="$1"
    echo "${subnet_id//\//_}" | sed 's/:/_/g'
}

# Get path to the node config generated by `ipc-cli subnet init`.
get_generated_node_config_path() {
    local subnet_id="$1"
    local ipc_config_dir
    ipc_config_dir=$(get_local_ipc_config_dir)
    local safe_subnet_id
    safe_subnet_id=$(sanitize_subnet_id_for_filename "$subnet_id")
    echo "$ipc_config_dir/node_${safe_subnet_id}.yaml"
}

# Extract latest gateway/registry addresses for a given subnet ID from IPC TOML config.
# Returns "<gateway>|<registry>" on stdout, or "|" when not found.
extract_parent_addresses_from_ipc_toml() {
    local ipc_config_file="$1"
    local target_subnet_id="$2"

    awk -v target="$target_subnet_id" '
        BEGIN {
            in_subnet = 0
            matched = 0
            current_id = ""
        ...
        in_subnet && /^id = / {
            ...
            sub(/^id = "/, "", current_id)
            sub(/"$/, "", current_id)
            matched = (current_id == target)
            next
        }
        in_subnet && /^gateway_addr = / {
            current_gateway = $0
            sub(/^gateway_addr = "/, "", current_gateway)
            sub(/"$/, "", current_gateway)
            if (matched) last_gateway = current_gateway
            next
        }
        in_subnet && /^registry_addr = / {
            current_registry = $0
            sub(/^registry_addr = "/, "", current_registry)
            sub(/"$/, "", current_registry)
            if (matched) last_registry = current_registry
            next
        }
        END {
            printf "%s|%s\n", last_gateway, last_registry
        }
    ' "$ipc_config_file"
}
...
generate_node_init_yml() {
    ...
    # Prefer the generated node config from `ipc-cli subnet init`, then patch it per-validator.
    local generated_node_config
    generated_node_config=$(get_generated_node_config_path "$subnet_id")
    if [ -f "$generated_node_config" ]; then
        cp "$generated_node_config" "$output_file"
        ...
        yq eval ".home = \"$node_home\"" -i "$output_file"
        yq eval ".key.\"wallet-type\" = \"evm\"" -i "$output_file"
        yq eval ".key.\"private-key\" = \"$private_key\"" -i "$output_file"
        yq eval "del(.key.path)" -i "$output_file"

        yq eval ".p2p.\"external-ip\" = \"$ip\"" -i "$output_file"
        if is_local_mode; then
            yq eval ".p2p.\"listen-ip\" = \"127.0.0.1\"" -i "$output_file"
        else
            yq eval ".p2p.\"listen-ip\" = \"0.0.0.0\"" -i "$output_file"
        fi
        yq eval ".p2p.ports.cometbft = $cometbft_p2p_port" -i "$output_file"
        yq eval ".p2p.ports.resolver = $libp2p_port" -i "$output_file"

        if [ -n "$peer_files" ]; then
            yq eval ".p2p.peers.\"peer-files\" = [\"$peer_files\"]" -i "$output_file"
        else
            yq eval ".p2p.peers = null" -i "$output_file"
        fi

        # If genesis is path-based, rewrite to where node init actually runs.
        local local_ipc_dir
        ...
        local existing_genesis_path
        existing_genesis_path=$(yq eval '.genesis.genesis // ""' "$output_file")
        if [ -n "$existing_genesis_path" ] && [ "$existing_genesis_path" != "null" ]; then
            yq eval ".genesis.genesis = \"$genesis_json_yml\"" -i "$output_file"
            yq eval ".genesis.sealed = \"$genesis_sealed_yml\"" -i "$output_file"
        fi

        log_info "Using generated node config template for $name"
        return 0
    fi
...
update_ipc_cli_configs() {
    log_info "Syncing IPC CLI configuration from local config store..."

    local ipc_config_dir
    ipc_config_dir=$(get_local_ipc_config_dir)
    local local_config="$ipc_config_dir/config.toml"
    if [ ! -f "$local_config" ]; then
        log_warn "Local IPC config not found at $local_config; skipping sync"
        return 0
    fi

    for idx in "${!VALIDATORS[@]}"; do
        local name="${VALIDATORS[$idx]}"
        local remote_ipc_dir=$(get_config_value "paths.ipc_config_dir")
        local remote_config_file=$(get_config_value "paths.ipc_config_file")

        remote_ipc_dir="${remote_ipc_dir/#\~/$HOME}"
        remote_config_file="${remote_config_file/#\~/$HOME}"

        log_info "Syncing IPC CLI config for $name..."
        exec_on_host_simple "$idx" "mkdir -p $remote_ipc_dir"
        if is_local_mode && [ "$local_config" = "$remote_config_file" ]; then
            log_info "Local IPC config already in place for $name"
        else
...
update_fendermint_topdown_config() {
    ...
    if [ -f "$ipc_config_file" ]; then
        local parent_addrs
        parent_addrs=$(extract_parent_addresses_from_ipc_toml "$ipc_config_file" "$parent_chain_id")
        parent_gateway="${parent_addrs%%|*}"
        parent_registry="${parent_addrs##*|}"
    fi
